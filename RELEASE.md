...
**Added:**
- **TEDAPI bearer auth and V2026_06 signed queries** — new `PW_TEDAPI_AUTH_MODE` (`basic` | `bearer`) and `PW_TEDAPI_API_VERSION` (`V2024_06` | `V2026_06`) settings, plus per-gateway `tedapi_auth_mode` / `tedapi_api_version` overrides in `PW_GATEWAYS` / the YAML config, are passed through to `pypowerwall.Powerwall()`. Bearer mode gives wired-LAN TEDAPI access on Powerwall 2 and solar-only gateways (not Powerwall 3 — use `rsa_key_path`), and `V2026_06` selects pypowerwall's Tesla-signed query set. Values are coerced leniently (a typo logs a warning and falls back instead of failing every poll), the server warns at registration when pypowerwall will ignore the request (Basic LAN, hybrid, cloud, or v1r + bearer) and when `V2026_06` is requested with a too-old protobuf, and once per gateway when the active transport differs from the requested one (e.g. bearer on a PW3). `/stats` reports both settings (`config.PW_TEDAPI_*`, top-level `tedapi_auth_mode` / `tedapi_api_version`, per-gateway requested/active values), `/health` adds `auth_mode` / `tedapi_api_version` per gateway, `/api/gateways` includes the resolved values, and the Console labels bearer gateways "TEDAPI (bearer)". CLI: `--tedapi-auth-mode`, `--tedapi-api-version`.
